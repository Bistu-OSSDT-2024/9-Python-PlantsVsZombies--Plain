...
#双头————————————————————————————————————————————————————————————————————————————————————————————————————————
class DoubleHeadSunflower(Plant):
    def __init__(self, x, y, sun_group):
        Plant.__init__(self, x, y, c.DOUBLEHEADSUNFLOWER, c.PLANT_HEALTH, None)
        self.sun_timer = 0
        self.sun_group = sun_group

    def idling(self):
        if self.sun_timer == 0:
            self.sun_timer = self.current_time - (c.FLOWER_SUN_INTERVAL - 3000)
        elif (self.current_time - self.sun_timer) > c.FLOWER_SUN_INTERVAL:
            self.sun_group.add(
                Sun(self.rect.centerx, self.rect.bottom, self.rect.right, self.rect.bottom + self.rect.h // 2))
            self.sun_timer = self.current_time
#戴夫———————————————————————————————————————————————————————————————————————————————————————
class CrazyDave(Plant):
    def __init__(self, x, y, bullet_group):
        Plant.__init__(self, x, y, c.CRAZYDAVE, c.PLANT_HEALTH, bullet_group)
        self.shoot_timer = 0

    def attacking(self):
        if (self.current_time - self.shoot_timer) > 10:
...
class PotatoMine(Plant):
    ...
    def canAttack(self, zombie):
        if (not self.is_init and zombie.rect.right >= 80 and
                (zombie.rect.x - self.rect.x) <= 80):
            return True
        return False
...
class Jalapeno(Plant):
    def __init__(self, x, y):
        ...
        self.explode_y_range = 10000
        self.explode_x_range = 10000
...
class WallNutBowling(Plant):
    def __init__(self, x, y, map_y, level):#初始化
        Plant.__init__(self, x, y, c.WALLNUTBOWLING, 1, None)
        self.map_y = map_y#初始位置
        self.level = level#等级
        self.init_rect = self.rect.copy()#植物初始矩形的位置和大小
        self.rotate_degree = 0#植物旋转的角度
        self.animate_interval = 200
        self.move_timer = 0
...
class RedWallNutBowling(Plant):#滚动坚果墙 js5
    def __init__(self, x, y):
